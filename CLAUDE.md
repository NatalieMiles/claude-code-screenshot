...
| Output to `~/Library/Caches/claude-code-screenshot/` (mode 700) with self-cleanup after 30 days | User-only readable — prevents other local accounts from reading screenshots that may contain sensitive on-screen content. macOS does **not** reliably clean `~/Library/Caches/` on its own, so the picker runs `find ... -mtime +30 -delete` on each invocation to keep the cache bounded. Retention window is hardcoded for v1; see GitHub issue for configurability tracking. | `/tmp/claude-screenshots/` (world-readable on shared Macs — prior default, fixed via SECURITY.md L1); Let directory grow unbounded (privacy leak + disk bloat over years of use) |
...
| Confirm captures via in-terminal text ack only (no popup, no notification) | Claude Code's TUI can't render inline terminal images (stdout → model context; `/dev/tty` races with TUI redraws). QuickLook steals focus on every capture. `osascript` notifications are unreliable on modern macOS (Sonoma+) — often silently suppressed even with "Allow notifications" toggled on, because Apple tightened rules for unsigned osascript calls. The `CAPTURED: <label>` line Claude prints back already confirms what was grabbed, reliably, everywhere. | QuickLook `qlmanage -p` (focus steal); `osascript` notifications (flaky on Sonoma+); iTerm/Kitty inline image protocols (break in Claude Code's TUI); `open -g Preview` (accumulates tabs) |
| Prefix picker items with type markers (🖥 screen, ★ frontmost window, ▫️ other window) | Closest thing to ChatGPT's per-item icons we can do inside `osascript choose from list` (text-only — no per-item `NSImage` support). Gives users a visual anchor for section + frontmost disambiguation without needing a compiled binary. | Ship a Swift NSMenu binary for real per-item app icons (breaks "zero compiled deps" rule; needs notarization for public distribution) |
| Slash command always Reads the captured PNG (drag-drop parity) | Users mentally model `/screenshot` as "hand this image to Claude" — identical to drag-dropping a file. Having a separate "don't Read" variant created confusion about why identical screenshots got different treatment. Unifying on Read-always removes the mental model split. | Two commands (`/screenshot` terse + `/screenshot-describe` describes) — caused confusion; Deferred-Read with token-savings — theoretical savings rarely realized because users usually do want Claude to look |
...
- [x] Apply security-review L1 (move output to `~/Library/Caches/`)
