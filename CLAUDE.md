...
| Output to `~/Library/Caches/claude-code-screenshot/` (mode 700) | User-only readable — prevents other local accounts from reading screenshots that may contain sensitive on-screen content. Caches path gets cleaned by macOS over time. | `/tmp/claude-screenshots/` (world-readable on shared Macs — prior default, fixed via SECURITY.md L1) |
